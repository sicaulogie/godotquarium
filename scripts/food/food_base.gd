...
func _physics_process(_delta):
	...
	# Fade out timer — from Food.cpp m0x180
	if fade_timer > 0:
		fade_timer -= 1
		# Fade alpha like Food.cpp Draw()
...
func _check_bottom():
	# From Food.cpp — food fades at bottom, star potion explodes
	if yd > 410.0:
		if food_type == 3:
			_explode()
		elif fade_timer < 1:
...
func _explode():
	# Star potion explosion — sound and particles handled by board later
	queue_free()
